...
def verify_sales_table(data):
    if isinstance(data, pd.DataFrame):
        ...
        if all(col in data.columns for col in required_columns):
            ...
            date_check = all(isinstance(data['Date'], pd.Series) and data['Date'].dtype == 'datetime64[ns]')
            quantity_check = all(data['Quantité'].apply(lambda x: isinstance(x, float)))
            reference_check = all(data['Reference'].apply(lambda x: bool(re.match(r'^[A-Za-z0-9]+$', str(x)))))
