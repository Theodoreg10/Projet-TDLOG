...
def verify_product_table(data):
    if isinstance(data, pd.DataFrame):
        ...
        if all(col in data.columns for col in required_columns):
            ...
            types_check = all(
                data['product_name'].apply(lambda x: isinstance(x, str)) and
                data['Qte_unitaire'].apply(lambda x: isinstance(x, float))
            )
            if types_check:
                return "La table de produits est conforme."
            else:
                return """Les types de données dans la table de
            produits ne correspondent pas aux critères."""
        else:
            return """La table de produits ne contient pas
        toutes les colonnes nécessaires (product_name, Qte_unitaire)."""
    else:
        return """Entrée non valide.
    Veuillez fournir une DataFrame (table de données)."""
...
def verify_sales_table(data):
    if isinstance(data, pd.DataFrame):
        ...
        if all(col in data.columns for col in required_columns):
            ...
            reference_check = all(data['Reference'].apply(
